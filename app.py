...
# Main section - M8 Course Presentation References
st.markdown('<p class="section-header">M8 Course Presentation References</p>', unsafe_allow_html=True)

# Login/Logout in the main section
col1, col2 = st.columns([5, 1])
with col2:
    if not st.session_state['authenticated']:
        show_login = st.button("🔑 Admin Login", use_container_width=True)
        if show_login:
...
# Admin actions when authenticated
if st.session_state['authenticated']:
    ...
    if not st.session_state['edit_id'] and not st.session_state['show_add_form']:
        if st.button("➕ Add New Reference", use_container_width=False):
            toggle_add_form()
...
if filtered_refs:
    for i, ref in enumerate(filtered_refs):
        with st.expander(f"📌 {ref['title']}", expanded=False):
            ...
            with col2:
                # Visit link button for everyone
                if st.button("🔗 Visit Link", key=f"url_{i}", use_container_width=True):
                    st.markdown(f"<script>window.open('{ref['url']}', '_blank');</script>", unsafe_allow_html=True)
                
                # Admin action buttons only shown when authenticated
                if st.session_state['authenticated']:
